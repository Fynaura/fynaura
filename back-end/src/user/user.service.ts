...
export class UserService {
    ...
    try {
      ...
      if (error.response?.data?.error?.message) {
        ...
        if (errorMessage.includes('EMAIL_NOT_FOUND')) {
          ...
          throw new HttpException('User account has been disabled', HttpStatus.FORBIDDEN);
        } else {
          throw new HttpException(errorMessage, HttpStatus.UNAUTHORIZED);
        }
      }
      
      // Generic error if none of the specific cases match
      throw new HttpException(
  ...
  private async sendPostRequest(url: string, data: any) {
    try {
      ...
      console.log('Error in sendPostRequest:', error.response?.data || error);
